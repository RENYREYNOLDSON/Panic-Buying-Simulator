...
class Shopper:
    def __init__(self,x,y,colour,number):
        ...
        self.y=y
        self.number=number
        self.water=random.randint(2,3)
        self.food=random.randint(2,3)
        self.hygiene=random.randint(2,3)
        self.panic=0
    ...
    def move(self):
        ...
        if self.stage=="home" and (self.food<3+self.panic or self.water<3+self.panic or self.hygiene<3+self.panic):
            self.stage="walking"
            ...
            if self.x==770++(self.number-1)*60 and self.y==840:
                self.stage="home"
            if self.x<770+(self.number-1)*60:
                self.x+=2
            elif self.x>770+(self.number-1)*60:
                self.x-=2
...
shoppers=[]
for i in range(1):
    shoppers.append(Shopper(770+i*60,840,(random.randint(0,255),random.randint(0,255),random.randint(0,255)),i+1))
